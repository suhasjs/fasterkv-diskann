...
/// (Logical) address into persistent memory. Identifies a page and an offset
/// within that page. Uses 48 bits: 25 bits for the offset and 23 bits for the
/// page. (The remaining 16 bits are reserved for use by the hash table.)
/// Address
class Address {
public:
  friend class PageOffset;

  /// An invalid address, used when you need to initialize an address but you
  /// don't have a valid value for it yet. NOTE: set to 1, not 0, to distinguish
  /// an invalid hash bucket entry (initialized to all zeros) from a valid hash
  /// bucket entry that points to an invalid address.
  static constexpr uint64_t kInvalidAddress = 1;

  /// A logical address is 8 bytes.
  /// --of which 48 bits are used for the address. (The remaining 16 bits are
  /// used by the hash table, for control bits and the tag.)
  ...
  /// Accessors.
  inline uint32_t page() const { return static_cast<uint32_t>(page_); }
  inline uint32_t offset() const { return static_cast<uint32_t>(offset_); }
  inline uint64_t control() const { return control_; }
...
public:
  ...
  /// Atomic access.
  inline Address load() const { return Address{control_.load()}; }
  inline void store(Address value) { control_.store(value.control()); }
  inline bool compare_exchange_strong(Address &expected, Address desired) {
    uint64_t expected_control = expected.control();
    bool result =
        control_.compare_exchange_strong(expected_control, desired.control());
    expected = Address{expected_control};
    return result;
  }

  /// Accessors.
  inline uint32_t page() const { return load().page(); }
