...
int main(int argc, char *argv[]) {
  ...
  if (argc < 10) {
    std::cout << "Usage (only float vectors supported): " << argv[0]
              << " <index_prefix> <max_num_pts> <query_bin> <gt_bin> <k> <L> "
                 "<beam_width> <num_threads> <insert_file> <num_inserts> <insert_R> <insert_L_index> <insert_alpha>"
              << std::endl;
    exit(0);
  }
  ...
  std::string query_bin = argv[3];
  std::string gt_bin = argv[4];
  uint32_t k_NN = std::stoi(argv[5]);
  uint32_t L_search = std::stoi(argv[6]);
  uint32_t beam_width = std::stoi(argv[7]);
  uint32_t num_threads = std::stoi(argv[8]);
  std::string insert_vec_file = argv[9];
  uint32_t num_inserts = std::stoi(argv[10]);
  uint32_t max_degree = std::stoi(argv[11]);
  uint32_t insert_L_index = std::stoi(argv[12]);
  float insert_alpha = std::stof(argv[13]);

  // create index
  diskann::FasterVamanaIndex index(max_num_points, max_degree, index_prefix);
