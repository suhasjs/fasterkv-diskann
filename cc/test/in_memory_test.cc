...
/// The hash always returns "0," so the FASTER store devolves into a linked
/// list.
TEST(InMemFaster, UpsertRead_DummyHash) {
  class DummyHash {
  public:
    inline size_t operator()(const uint16_t &key) const { return 42; }
  };
  ...
  public:
    ...
    inline bool PutAtomic(Value &value) {
      ...
      do {
        ...
        // Try to get the lock.
        success =
            value.length_.compare_exchange_weak(expected_length, UINT8_MAX);
      } while (!success);
  ...
  auto upsert_worker = [&store](size_t thread_idx) {
    ...
    for (size_t idx = 0; idx < kNumOps; ++idx) {
      ...
      UpsertContext context{
          static_cast<uint32_t>((thread_idx * kNumOps) + idx)};
      Status result = store.Upsert(context, callback, 1);
      ASSERT_EQ(Status::Ok, result);
    }
...
TEST(InMemFaster, UpsertRead_ResizeValue_Concurrent) {
  ...
  public:
    ...
    /// The implicit and explicit interfaces require a key() accessor.
    inline const Key &key() const { return key_; }
    inline uint32_t value_size() const { return sizeof(Value) + length_; }
    /// Non-atomic and atomic Put() methods.
    inline void Put(Value &value) {
...
TEST(InMemFaster, Rmw) {
  ...
  public:
    ...
    inline void RmwInitial(Value &value) { value.value = incr_; }
...
TEST(InMemFaster, Rmw_Concurrent) {
  ...
  for (size_t idx = 0; idx < kRange; ++idx) {
    ...
    ASSERT_EQ(Status::Ok, result) << idx;
    // Should have performed 4 RMWs.
    ASSERT_EQ((kNumThreads * (kNumThreads - 1)) * (kNumRmws / kRange),
              context.output);
  }
  ...
  for (size_t idx = 0; idx < kRange; ++idx) {
    ...
    ASSERT_EQ(Status::Ok, result);
    // All upserts should have inserts (non-atomic).
    ASSERT_EQ(((kNumThreads * (kNumThreads - 1)) / 2) * (kNumRmws / kRange),
              context.output);
  }
...
TEST(InMemFaster, Rmw_ResizeValue_Concurrent) {
  ...
  private:
    ...
    inline int8_t *buffer() { return reinterpret_cast<int8_t *>(this + 1); }
  };
  ...
  public:
    ...
    /// The implicit and explicit interfaces require a key() accessor.
    inline const Key &key() const { return key_; }
    inline uint32_t value_size() const { return sizeof(value_t) + length_; }
    ...
    inline void RmwCopy(const Value &old_value, Value &value) {
      ...
      value.length_ = length_;
      std::memset(value.buffer(), incr_, length_);
      for (uint32_t idx = 0; idx < std::min(old_value.length_, length_);
           ++idx) {
  ...
  auto rmw_worker = [&store](size_t thread_idx, int8_t incr,
...
TEST(InMemFaster, ConcurrentDelete) {
  ...
  public:
    ...
    inline void RmwInitial(Value &value) { value.value = 1; }
    ...
    inline bool RmwAtomic(Value &value) {
      // Not supported: so that operation would allocate a new entry for the
      // update.
      return false;
    }
...
TEST(InMemFaster, GrowHashTable) {
  ...
  auto rmw_worker = [&store](size_t thread_idx, int64_t multiplier) {
    ...
    if (thread_idx == 0) {
      ...
      store.GrowIndex([](uint64_t new_size) { grow_done = true; });
    }
...
TEST(InMemFaster, UpsertRead_VariableLengthKey) {
  ...
  public:
    ...
    inline bool operator==(const Key &other) const {
      if (this->key_length_ != other.key_length_)
        return false;
  ...
  public:
    ...
    inline uint32_t size() const { return Key::size(key_length_); }
    inline KeyHash GetHash() const {
  ...
  public:
    ...
    UpsertContext(uint8_t *key, uint32_t key_length) : key_{key, key_length} {}

    /// Copy (and deep-copy) constructor.
    UpsertContext(const UpsertContext &other) : key_{other.key_} {}

    /// The implicit and explicit interfaces require a key() accessor.
    inline const ShallowKey &key() const { return key_; }
    inline static constexpr uint32_t value_size() { return sizeof(value_t); }
    /// Non-atomic and atomic Put() methods.
    inline void Put(Value &value) { value.value = 23; }
    inline bool PutAtomic(Value &value) {
  ...
  protected:
    /// The explicit interface requires a DeepCopy_Internal() implementation.
    Status DeepCopy_Internal(IAsyncContext *&context_copy) {
      // In this particular test, the key content is always on the heap and
      // always available, so we don't need to copy the key content. If the key
      // content were on the stack, we would need to copy the key content to the
      // heap as well
      //
      return IAsyncContext::DeepCopy_Internal(*this, context_copy);
    }
  ...
  public:
    ...
    ReadContext(uint8_t *key, uint32_t key_length) : key_{key, key_length} {}
